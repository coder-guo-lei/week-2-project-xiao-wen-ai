...
const COLOR_PALETTE = [
  ...
  { color: '#00D4AA', weight: 0.20 }, // 青绿 20%
  { color: '#FFFFFF', weight: 0.20 }, // 白色星光 20%
];
...
export default function LoginParticleBg({ className = '' }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      sizes[i] = isWhite
        ? 0.02 + Math.random() * 0.01  // 0.02 - 0.03
        : 0.008 + Math.random() * 0.007; // 0.008 - 0.015
    }
  ...
  return (
    <div
      ref={containerRef}
      className={`login-particle-bg ${className}`.trim()}
    />
  );
}
