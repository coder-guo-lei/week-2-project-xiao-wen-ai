...
export async function exportChatDocx(history, options = {}) {
  ...
  const children = [
    new Paragraph({
      ...
      children: [new TextRun({ text: '小文 AI 对话记录', bold: true, size: 44, font: 'Microsoft YaHei' })],
    }),
    new Paragraph({
      spacing: { after: 120 },
      children: [
  ...
  turns.forEach((turn, idx) => {
    children.push(
      new Paragraph({
        ...
        children: [new TextRun({ text: `第 ${idx + 1} 轮`, bold: true, size: 28, font: 'Microsoft YaHei' })],
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        spacing: { after: 80 },
        children: [new TextRun({ text: '用户', bold: true, size: 24, color: USER_COLOR, font: 'Microsoft YaHei' })],
      }),
      ...paragraphsFromText(turn.user, { spacingAfter: 160 }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        spacing: { before: 80, after: 80 },
        children: [new TextRun({ text: '小文', bold: true, size: 24, color: ASSISTANT_COLOR, font: 'Microsoft YaHei' })],
      }),
